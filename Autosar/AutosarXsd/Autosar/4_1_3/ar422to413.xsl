	...
	<xsl:template match="AR:AUTOSAR">
		<xsl:choose>
			<xsl:when test="contains(string(@*), 'AUTOSAR_4-2-2.xsd')">
				...
				<AUTOSAR xsi:schemaLocation="{concat('http://autosar.org/schema/r4.0 ', concat(substring-before($ARPATH, '4-2-2'), '4-1-3', substring-after($ARPATH, '4-2-2') ))}" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://autosar.org/schema/r4.0">
					<xsl:apply-templates/>
				</AUTOSAR>
			</xsl:when>
			<xsl:otherwise>
				<AUTOSAR xsi:schemaLocation="http://autosar.org/schema/r4.0 AUTOSAR_4-1-3.xsd" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="http://autosar.org/schema/r4.0">
					<xsl:apply-templates/>
				</AUTOSAR>
			</xsl:otherwise>
		</xsl:choose>
