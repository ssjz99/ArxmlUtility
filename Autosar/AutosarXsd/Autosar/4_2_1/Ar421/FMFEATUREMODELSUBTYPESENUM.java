//
// ????? JavaTM Architecture for XML Binding (JAXB) ???? v2.2.8-b130911.1802 ???
// ??? <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// ?????????, ??????????????
// ????: 2018.03.12 ?? 11:42:22 PM CET 
//


package Ar421;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>FM-FEATURE-MODEL--SUBTYPES-ENUM? Java ??
 * 
 * <p>????????????????????
 * <p>
 * <pre>
 * &lt;simpleType name="FM-FEATURE-MODEL--SUBTYPES-ENUM">
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string">
 *     &lt;enumeration value="FM-FEATURE-MODEL"/>
 *   &lt;/restriction>
 * &lt;/simpleType>
 * </pre>
 * 
 */
@XmlType(name = "FM-FEATURE-MODEL--SUBTYPES-ENUM")
@XmlEnum
public enum FMFEATUREMODELSUBTYPESENUM {

    @XmlEnumValue("FM-FEATURE-MODEL")
    FM___FEATURE___MODEL("FM-FEATURE-MODEL");
    private final String value;

    FMFEATUREMODELSUBTYPESENUM(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static FMFEATUREMODELSUBTYPESENUM fromValue(String v) {
        for (FMFEATUREMODELSUBTYPESENUM c: FMFEATUREMODELSUBTYPESENUM.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
