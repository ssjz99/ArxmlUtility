//
// ????? JavaTM Architecture for XML Binding (JAXB) ???? v2.2.8-b130911.1802 ???
// ??? <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// ?????????, ??????????????
// ????: 2018.03.12 ?? 11:42:22 PM CET 
//


package Ar421;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>FM-FEATURE--SUBTYPES-ENUM? Java ??
 * 
 * <p>????????????????????
 * <p>
 * <pre>
 * &lt;simpleType name="FM-FEATURE--SUBTYPES-ENUM">
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string">
 *     &lt;enumeration value="FM-FEATURE"/>
 *   &lt;/restriction>
 * &lt;/simpleType>
 * </pre>
 * 
 */
@XmlType(name = "FM-FEATURE--SUBTYPES-ENUM")
@XmlEnum
public enum FMFEATURESUBTYPESENUM {

    @XmlEnumValue("FM-FEATURE")
    FM___FEATURE("FM-FEATURE");
    private final String value;

    FMFEATURESUBTYPESENUM(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static FMFEATURESUBTYPESENUM fromValue(String v) {
        for (FMFEATURESUBTYPESENUM c: FMFEATURESUBTYPESENUM.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
