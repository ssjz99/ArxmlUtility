//
// ????? JavaTM Architecture for XML Binding (JAXB) ???? v2.2.8-b130911.1802 ???
// ??? <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// ?????????, ??????????????
// ????: 2018.03.12 ?? 11:42:22 PM CET 
//


package Ar421;

import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>COUPLING-PORT-FIFO--SUBTYPES-ENUM? Java ??
 * 
 * <p>????????????????????
 * <p>
 * <pre>
 * &lt;simpleType name="COUPLING-PORT-FIFO--SUBTYPES-ENUM">
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string">
 *     &lt;enumeration value="COUPLING-PORT-FIFO"/>
 *   &lt;/restriction>
 * &lt;/simpleType>
 * </pre>
 * 
 */
@XmlType(name = "COUPLING-PORT-FIFO--SUBTYPES-ENUM")
@XmlEnum
public enum COUPLINGPORTFIFOSUBTYPESENUM {

    @XmlEnumValue("COUPLING-PORT-FIFO")
    COUPLING___PORT___FIFO("COUPLING-PORT-FIFO");
    private final String value;

    COUPLINGPORTFIFOSUBTYPESENUM(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static COUPLINGPORTFIFOSUBTYPESENUM fromValue(String v) {
        for (COUPLINGPORTFIFOSUBTYPESENUM c: COUPLINGPORTFIFOSUBTYPESENUM.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
